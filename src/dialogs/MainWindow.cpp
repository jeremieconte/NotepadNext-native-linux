    ...
    zoomEventWatcher(new ZoomEventWatcher(this))
{
    ...
    // Re-theme live when the user changes the colour scheme in Preferences...
    connect(app->getSettings(), &ApplicationSettings::colorSchemeChanged, this, [=]() {
        applyStyleSheet();
        app->getEditorManager()->applyColorScheme();
    });
    // ...and when the desktop (e.g. GNOME 50 "Dark Style") toggles, but only
    // while we are following the system scheme. We listen both to Qt's own
    // signal and directly to the freedesktop appearance portal, since Qt does
    // not always surface the GNOME preference through QStyleHints.
    connect(QGuiApplication::styleHints(), &QStyleHints::colorSchemeChanged, this, [=]() {
        if (app->getSettings()->colorScheme() == ApplicationSettings::SystemColorScheme) {
            applyStyleSheet();
            app->getEditorManager()->applyColorScheme();
        }
    });
    QDBusConnection::sessionBus().connect(
        ...
        this, SLOT(desktopPortalSettingChanged(QString, QString)));
...
void MainWindow::desktopPortalSettingChanged(const QString &nspace, const QString &key)
{
    if (nspace == QLatin1String("org.freedesktop.appearance")
        && key == QLatin1String("color-scheme")
        && app->getSettings()->colorScheme() == ApplicationSettings::SystemColorScheme) {
        applyStyleSheet();
        app->getEditorManager()->applyColorScheme();
    }
}
...
void MainWindow::applyStyleSheet()
{
    ...
    ApplicationSettings *appSettings = app->getSettings();

    // Resolve the effective theme (honours the portal/GNOME preference in
    // "System" mode) and drive Qt's native colour scheme so that all standard
    // widgets (menus, toolbars, dialogs, the Preferences window, scrollbars,
    // ...) follow it -- keeping the native chrome consistent with our own
    // stylesheet and editor colours.
    const bool dark = appSettings->isDarkMode();
    QGuiApplication::styleHints()->setColorScheme(dark ? Qt::ColorScheme::Dark : Qt::ColorScheme::Light);

    QString sheet;
    QFile f(dark ? QStringLiteral(":/stylesheets/npp-dark.css")
