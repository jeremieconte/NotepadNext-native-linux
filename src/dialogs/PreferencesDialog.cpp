    ...
    settings(settings)
{
    ...
    ui->comboBoxColorScheme->addItem(tr("System"), ApplicationSettings::SystemColorScheme);
    ui->comboBoxColorScheme->addItem(tr("Light"), ApplicationSettings::LightColorScheme);
    ui->comboBoxColorScheme->addItem(tr("Dark"), ApplicationSettings::DarkColorScheme);
    {
        int idx = ui->comboBoxColorScheme->findData(settings->colorScheme());
        ui->comboBoxColorScheme->setCurrentIndex(idx == -1 ? 0 : idx);
    }
    connect(ui->comboBoxColorScheme, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [=](int index) {
        settings->setColorScheme(static_cast<ApplicationSettings::ColorSchemeEnum>(
            ui->comboBoxColorScheme->itemData(index).toInt()));
    });
    connect(settings, &ApplicationSettings::colorSchemeChanged, this, [=](ApplicationSettings::ColorSchemeEnum scheme) {
        int idx = ui->comboBoxColorScheme->findData(scheme);
        ui->comboBoxColorScheme->setCurrentIndex(idx == -1 ? 0 : idx);
    });
