...
// Applies the theme-dependent editor colours. Scintilla style colours are
// 0xBBGGRR; element colours are 0xAABBGGRR. Called at editor creation and
// again (with colourise()) when the colour scheme changes at runtime.
static void applyEditorColours(ScintillaNext *editor, bool dark, ApplicationSettings *settings)
{
    const int fore = dark ? 0xD4D4D4 : 0x000000; // grayscale: channel order irrelevant
    const int back = dark ? 0x1E1E1E : 0xFFFFFF;

    editor->styleSetFore(STYLE_DEFAULT, fore);
    editor->styleSetBack(STYLE_DEFAULT, back);
    editor->styleSetSize(STYLE_DEFAULT, settings->fontSize());
    editor->styleSetFont(STYLE_DEFAULT, settings->fontName().toUtf8().data());
    editor->styleClearAll();

    editor->styleSetFore(STYLE_LINENUMBER, dark ? 0x9A9A9A : 0x808080);
    editor->styleSetBack(STYLE_LINENUMBER, dark ? 0x2B2B2B : 0xE4E4E4);
    editor->styleSetBold(STYLE_LINENUMBER, false);

    editor->styleSetFore(STYLE_BRACELIGHT, dark ? 0xFFB060 : 0x0000FF);
    editor->styleSetBack(STYLE_BRACELIGHT, back);
    editor->styleSetFore(STYLE_BRACEBAD, dark ? 0x5050FF : 0x000080);
    editor->styleSetBack(STYLE_BRACEBAD, back);

    editor->styleSetFore(STYLE_INDENTGUIDE, dark ? 0x505050 : 0xC0C0C0);
    editor->styleSetBack(STYLE_INDENTGUIDE, back);

    editor->setElementColour(SC_ELEMENT_SELECTION_INACTIVE_BACK, dark ? 0xFF404040u : 0xFFE0E0E0u);
    editor->setElementColour(SC_ELEMENT_CARET_LINE_BACK,         dark ? 0xFF2A2A2Au : 0xFFFFE8E8u);
    editor->setElementColour(SC_ELEMENT_WHITE_SPACE,             dark ? 0xFF585858u : 0xFFD0D0D0u);
    editor->setElementColour(SC_ELEMENT_FOLD_LINE,               dark ? 0xFF585858u : 0xFFA0A0A0u);
    if (dark)
        editor->setElementColour(SC_ELEMENT_CARET, 0xFFD4D4D4u);
    else
        editor->resetElementColour(SC_ELEMENT_CARET);

    editor->setFoldMarginColour(true, back);
    editor->setFoldMarginHiColour(true, dark ? 0x2B2B2B : 0xE9E9E9);

    editor->setEdgeColour(dark ? 0x605040 : 0x80FFFF);
}

void EditorManager::applyColorScheme()
{
    const bool dark = settings->isDarkMode();

    purgeOldEditorPointers();
    for (const QPointer<ScintillaNext> &editor : getEditors()) {
        if (editor.isNull())
            continue;
        applyEditorColours(editor, dark, settings);
        // Re-run the lexer so language styles are re-applied on top of the
        // freshly cleared (styleClearAll) base colours.
...
void EditorManager::setupEditor(ScintillaNext *editor)
{
    ...
    // Base/style colours are theme-dependent (light or dark). This also runs
    // styleSetSize/Font(STYLE_DEFAULT) + styleClearAll() for us.
    applyEditorColours(editor, settings->isDarkMode(), settings);
