...
public:
    ...
    // Application colour theme. "System" follows the desktop's dark-style
    // preference (on GNOME this is the freedesktop org.freedesktop.appearance
    // color-scheme reported by Qt via QStyleHints::colorScheme()).
    enum ColorSchemeEnum {
        SystemColorScheme,
        LightColorScheme,
        DarkColorScheme
    };
    Q_ENUM(ColorSchemeEnum)

    // Resolves the effective theme: honours an explicit Light/Dark choice,
    // otherwise follows the system (GNOME) dark-style preference.
    ...
    DEFINE_SETTING(ColorScheme, colorScheme, ColorSchemeEnum)
